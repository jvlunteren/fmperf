...
def get_streaming_response(response: requests.Response):
    ...
    for chunk in response.iter_lines(
        ...
        if chunk and not finished:
            ...
            if (out['logprobs'] == None) or (len(out['logprobs']['tokens']) == 1):
                if out['text'] != '': # filter empty tokens
                    out['logprobs'] = None # not needed
                    yield out
            else:
                for i, t in enumerate(out['logprobs']['tokens']):
                    if t != '': # filter empty tokens
                        yield { # yield sequences comprised of multiple tokens generated by speculation one-by-one
                            'index': 0,
                            'text': t,
                            'logprobs': None,
                            'finish_reason': None if (i < len(out['logprobs']['tokens']) - 1) else out['finish_reason'],
                            'stop_reason': None
                        }
